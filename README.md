...
> 전시의 컨셉이 어둠인 만큼 색은 제한적으로 사용하였다. 그리고 전시의 취지에 맞게 시각 장애인이나 저시력자도 홈페이지 이용에 문제 없도록 접근성에 평소보다 더 신경써서 작업을 진행하였다. 단일 페이지 웹사이트지만 내용을 스크롤하지 않고도 쉽고 빠르게 탐색이 가능하도록 우측에 네비게이션을 배치하여 자바스크립트를 이용하여 각 내용의 위치로 바로 이동할 수 있게 하였다. 모바일 기기에서는 화면을 좀 더 효율적으로 사용할 수 있게 네비게이션을 배치하지 않았다.  
